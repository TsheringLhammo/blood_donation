...
try {
    ...
    if ($appointment) {
        try {
            // Try to find donor by phone number or email from appointment
            $donorStmt = $pdo->prepare('SELECT id FROM tbldonors WHERE phone = ? OR email = ? LIMIT 1');
            $donorStmt->execute([$appointment['phone_number'] ?? '', $appointment['full_name'] . '@example.com']);
            ...
            if ($donor) {
                ...
                $message = "Your appointment on {$date} at {$time} at {$bank} has been confirmed.";
                
                $notifStmt = $pdo->prepare(
                    'INSERT INTO tblnotifications (donor_id, title, message, type, severity, channel, is_read)
                     VALUES (?, ?, ?, "appointment", "success", "in_app", 0)'
                );
                $notifStmt->execute([
                    $donor['id'],
                    $title,
